...
Deploy the SAM application with parameter overrides.
...
```bash
sam-send-emails$ sam logs -n HelloWorldFunction --stack-name sam-send-emails --tail
```
...
```bash
aws s3 cp ses-test/tokyo-skytree.pdf s3://beralfonx-temporal/files/tokyo-skytree.pdf
aws s3 cp ses-test/japan-eng.pdf s3://beralfonx-temporal/files/japan-eng.pdf
```
